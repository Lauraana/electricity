...
# LIGHTING OP: 
  # Routine inpatient care or overnight in all facilities
      ...
      mwfac$hours24overall[mwfac$V101A == "No 24 hour staff"] <- 0
          ...
          regframeoutpover <- data.frame(mwfac$V102T, 
                                         mwfac$hours24overall, 
                                         energycatframe3$encate3, 
                                         lightcatframe$licatopdfunc, 
                                         mwfac$inpatorovern, 
                                         facilitycatwho$facilitycategory, 
                                         mwfac$V008,  mwfac$V001, mwfac$V003)
    
          sapply(regframeoutpover, function(x) sum(is.na(x))) # n.a. inpat=83; encate3=88 ("don´t know if connected")
          sapply(regframeoutpover, function(x) length(unique(x)))
          
          # Delete missings
          regframeoutpover <- na.omit(regframeoutpover)
          
          modeloutpover <- glm(mwfac.inpatorovern~.,family=binomial(link='logit'),data=regframeoutpover)
          summary(modeloutpover)
          ...
          # Kann	das	angenommene	Regressionsmodell	mehr	als	das	Null-Modell	erklären?	
          # likelihood ratio test: difference between the null deviance and the residual deviance: shows how the model is doing against the null model (the wider the gap the better)
          anova(modeloutpover, test="Chisq") 
          
          # "Wie	viel	Varianz	kann	durch	das	Regressionsmodell	aufgeklärt	werden?"	
          # Model fit:  McFadden, Cox and Snell and Nagelkerke: 0.2: ok, 0.4: good 
          install.packages("rcompanion")
          library(rcompanion)
          nagelkerke(modeloutpover)
          ...
          # delta 1: runs 972 times = biased estimate of true; second: correction for the true error
          predabil <- function(r, pi = 0) mean(abs(r-pi) > 0.5)
          cv.glm(regframeoutpover, modeloutpover, predabil, 972)
          
          
          library(stargazer) 
          stargazer(modeloutpover, type = "html",
                    dep.var.labels=c("Inpatient or overnight observation services offered (=1)"),
                    covariate.labels=c("Full time personnel",
                                       "Personnel 24 hours available/on call (=1)",
                                       "Interrupt. grid, w/ back-up (Ref.: Uninterrupt. grid w/ back-up)", "Uninterrupt. grid, w/o back-up (Ref.: Uninterrupt. grid w/ back-up)", "Interrupt. grid, w/o back-up (Ref.: Uninterrupt. grid w/ back-up)", "Off-grid (Ref.: Uninterrupt. grid w/ back-up)", "No electricity source (Ref.: Uninterrupt. grid w/ back-up)", 
                                       "Lighting in outpatient area functional (=1)",
                                       "First Level (ref.: Community Level)", "Referral Level (ref.: Community Level)", 
                                       "Christian Health Association of Malawi (CHAM) (Ref.: Government)", "Private for profit (Ref.: Government)", "Mission/Faith-based (other than CHAM) (Ref.: Government)", "NGO (Ref.: Government)", "Company (Ref.: Government)", 
                                       "Central region (Ref.: Northern region)", "Southern region (Ref.: Northern region)",
                                       "Rural facility (Ref.: Urban facility)"), out="Outc_INPATOVERNIGHT.htm")
  ...
  # Nr of outpatient visits
      ...
      # Regframe
      # Func
      regframenoutp <- data.frame(mwfac$V102T, mwfac$licatopdfunc, mwfac$V134, mwfac$encate3, mwfac$facilitycategory, mwfac$V008,  mwfac$V001, mwfac$V003)  
      # Av
      ...
      # Missing values
      sapply(regframenoutp, function(x) sum(is.na(x))) # n.a. inp/overnight=113+83; encate3=88 ("don´t know if connected")
      ...
      summary(modeloutpno)
          # Energy sign, light insign
      
      # Model diagnostics
   
          # Linearity, no heteroscedasticity: Residual plot
          plot(modeloutpno, which=1)
          
          # Linearity of single variables ("personnel")
          ...
          # Outliers
          plot(modeloutpno,5)
          # one outlier ? - how determine it? 
          
          # Autocorrelation ? (Fehlerterme unabhängig)
          ...
          # (Without light)
          regframenoutpA <- data.frame(mwfac$V102T, mwfac$V134, mwfac$encate3, mwfac$facilitycategory, mwfac$V008,  mwfac$V001, mwfac$V003)  
          
          
          # (delete missings)
          regframenoutpA <- na.omit(regframenoutpA)
          regframenoutpA <- regframenoutpA[regframenoutpA$mwfac.V134 != 99998,]
          
          # (model)
          modeloutpnoA <- lm(mwfac.V134~.,data=regframenoutpA)
          summary(modeloutpnoA)
          #energy sign, lighting insign
          
          # (Without energy)
          regframenoutpB <- data.frame(mwfac$V102T, mwfac$V134, mwfac$licatopdfunc, mwfac$facilitycategory, mwfac$V008,  mwfac$V001, mwfac$V003)  
          
          
          # (delete missings)
          regframenoutpB <- na.omit(regframenoutpB)
          regframenoutpB <- regframenoutpB[regframenoutpB$mwfac.V134 != 99998,]
          
          # (model)
          modeloutpnoB <- lm(mwfac.V134~.,data=regframenoutpB)
          summary(modeloutpnoB)
          # ligthing not significant
